...
var OBJECT_TYPES = ["truck", "boat", "package", "location"];
...
function saveObject() {
	try {
	    throw "saveObject() not implemented";
...
function exportModel() {
	try {
	    throw "exportModel() not implemented";
...
function parseData(data) {
	return {
		nodes: parseNodes(data.objects.location),
		agents: parseAgents(data.objects.truck, data.objects.boat, data.objects.package),
		edges: parseEdges(data.graph.edges)
	}
}


function parseNodes(locations) {
	var nodes = {};
	for (var key in locations) {
		nodes[key] = parseNode(key, locations[key]);
	}
	return nodes;
}
...
function parseAgents(trucks, boats, packages) {
	var agents = {};
	$.each(trucks, function (key, value) {
		agents[key] = new Agent("truck", value.at[1]);
	});
	$.each(boats, function (key, value) {
		agents[key] = new Agent("boat", value.at[1]);
	});
	$.each(packages, function (key, value) {
		agents[key] = new Agent("package", value.at[1]);
		if (value.deliverable !== undefined && value.at[1] !== value.deliverable[1]) {
		    agents[key+"-dest"] = new Agent("package_dest", value.deliverable[1]);
		}
	});
	return agents;
}
...
function parseEdge(from, to, edge) {
	var combined = $.extend({}, edge);
	var type = null;
	if (combined["connected-by-sea"] === true) {
	    type = "sea";
	} else if (combined["connected-by-land"] === true) {
	    type = "land";
	}
	return new Edge(from, to, combined["travel-time"], type);
}
...
function createEdgeElements(edges, nodes, map) {
	...
	$.each(edges, function (key, value) {
		...
		e.attr("x2", x2);
		e.attr("y2", y2);
		e.attr("style", "stroke:"+map.edge.colour[value.type]+";stroke-width:"+map.edge.width);
		e.attr("onclick", "displayObjectData('"+key+"');");
		edge_elements[key] = e;
...
function createAgentElements(agents, nodes, map) {
    ...
    var shorten_name = function (name) {
        var end = /[0-9]+(-dest)?$/.exec(name);
        return name.charAt(0) + end[0];
    };
	$.each(node_population, function (key, value) {
		...
		agent_elements[key+"-agents"] = createTextElement(value.map(shorten_name).join(", "), x, y, map);
	});
...
function Edge(from, to, distance, type) {
	this.from = from;
	this.to = to;
